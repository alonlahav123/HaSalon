...
function UserEntry(props){
    ...
    return(
        <Card>
            ...
            <Card.Body>
            <b>Name: </b>{userName}, <b>Email: </b>{userEmail}, <br />
            <b>Balance: </b>{userCouchPoints} <br />
            <InputGroup>
                <FormControl aria-describedby="basic-addon2" onChange={event => setUserInput(event.target.value)} value={userInput} type="number"/>
                <InputGroup.Append> 
                <Button variant="outline-secondary" onClick={event => updatePoints(userInput,-1,userCouchPoints)}>-</Button>
                <Button variant="outline-secondary" onClick={event => updatePoints(userInput,1,userCouchPoints)}>+</Button>
                </InputGroup.Append>
            </InputGroup>
            </Card.Body>
...
function IsAdmin(props){
    ...
    DB.collection('users').get().then(function (querySnapshot) {
        querySnapshot.forEach(function (doc) {
            ...
            output.push(<UserEntry key={`user_${doc.id}`} userName={doc.data().userName} userEmail={doc.data().userEmail} userCouchPoints={doc.data().userCouchPoints} userID={doc.id} />);
        })}).then(function(){
        console.log("weDone")
    ...
    return(
                    <div>
                        <Card>
                            ...
                            <Card.Body>
                                {isLoading ? <div>{output}</div> : <button onClick={() => setIsLoading(true)}>Load Users</button>}
                            </Card.Body>
                        </Card>                        
                    </div>
    );
